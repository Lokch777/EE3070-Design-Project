...
**文字訊息（ESP32 -> 伺服器）**：
- `{"type":"pong"}`: heartbeat 回覆
- `{"type":"playback_complete","request_id":"..."}`: 回報播放完成
- `{"type":"request_more_audio","request_id":"...","buffer_available":1234}`: 回報緩衝狀態
...
**Query 參數（optional）**：
- `device_id`: 裝置識別碼（搭配 `capture_requested.data.device_id` 做路由）
