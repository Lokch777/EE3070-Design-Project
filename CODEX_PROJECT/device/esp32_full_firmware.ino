/*
 * ESP32 ASR Capture Vision MVP - Complete Firmware
 * 
 * Features:
 * - I2S microphone audio streaming
...
void setup() {
  ...
  Serial.println("\n✓ Setup complete!");
  Serial.println("Streaming audio and waiting for CAPTURE commands...\n");
}
...
void handleCaptureCommand(String message) {
  ...
  // Parse JSON
  // Expected format: {"type":"CAPTURE","req_id":"xxx","timestamp":123}
  int reqIdStart = message.indexOf("\"req_id\":\"") + 10;
  int reqIdEnd = message.indexOf("\"", reqIdStart);
  String reqId = message.substring(reqIdStart, reqIdEnd);
...
void streamAudio() {
  ...
  esp_err_t result = i2s_read(I2S_PORT, audioBuffer, AUDIO_BUFFER_SIZE, &bytesRead, portMAX_DELAY);
