...
// ===== Phase 2 =====
#define CAMERA_STREAM_INTERVAL_MS  1000
...
void setup() {
  ...
  // ---Phase2---
  xTaskCreatePinnedToCore(cameraStreamTask, "camStream", 4096, nullptr, 1, nullptr, 1);
  ...
  wsAudio.onMessage([](WebsocketsMessage message) {
    ...
    // ── 新增這一段：Phase 1 Barge-in 處理 ──
    if (strcmp(type, "stop_playback") == 0) {
      Serial.println("[BARGE-IN] Stop playback received. Clearing buffer...");
      
      // 呼叫現有的重置函式：清空環形緩衝區並重置狀態
      // 參數 true 代表連同 request_id 一起清除
      resetPlaybackBuffer(true);
      
      // 注意：因為你的 FIX 2 設計中，麥克風是持續收音的
      // (沒有被 speakerSessionActive 阻擋)，所以這裡清空播放後，
      // 使用者的聲音就會繼續送給後端，達成自然的對話打斷。
      return;
    }
    // ───────────────────────────────────────────
...
// ============================================================
// 🌟 Phase 2: 新增的相機串流任務 (1 FPS)
void cameraStreamTask(void* param) {
    uint32_t lastCaptureMs = 0;

    while (true) {
        // 確保 WebSocket 與音訊已連線才開始傳送影像
        if (!wsCamera.available() || !audioConnected) {
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        }

        uint32_t now = millis();
        if ((now - lastCaptureMs) < CAMERA_STREAM_INTERVAL_MS) {
        ...
        // 確保影像大小符合 Omni 限制，並且在預算內 (QVGA 通常 < 15KB)
        if (fb->len <= (80 * 1024)) { 
            wsCamera.sendBinary((const char*)fb->buf, fb->len);
            Serial.printf("[CAM] Frame sent: %u bytes\n", fb->len); // 方便你 Debug 看有沒有成功送出
        }
